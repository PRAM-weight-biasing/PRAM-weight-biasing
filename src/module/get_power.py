...
# ------------------------------
CONVERTER_MAP = {
    "single": SinglePairConductanceConverter,
...
def _build_converter(cfg: Dict[str, Any]):
    """
    cfg keys (examples):
      converter: one of {"single", "mapped", "mapped_1yr", ...}
      g_max, g_min, distortion_f
      f_lst (for npair/custom), g_lst (for custom)
    """
    ctype = cfg.get("converter", "single").lower()
...
def analyze_array_energy(
    ...
    config: - device: "cuda" or "cpu" 
            ...
            - converter: {"single","mapped","mapped_1yr","npair","custom"} (plus converter-specific params like g_max/g_min/distortion_f/f_lst/g_lst) 
    ...
    for k in layer_inputs:
        layer_inputs[k] = torch.cat(layer_inputs[k], dim=0).to(device)
